...
NoteEditor 是一个多阶段流水线 CLI 工具，每阶段封装为独立模块，由 Pipeline Orchestrator 协调调度。
...
└──┬────┬────┬────┬────┬────┬────┬────┬───────────────────┘
   ↓    ↓    ↓    ↓    ↓    ↓    ↓    ↓
 ┌────┐┌────┐┌────┐┌────┐┌────┐┌────┐┌────┐┌────┐
 │Par ││Lay ││OCR ││Img ││Fnt ││Sty ││Bkg ││Bld │  ← stages/
 │ser ││out ││    ││    ││    ││le  ││    ││er  │
 └────┘└────┘└────┘└────┘└────┘└────┘└────┘└────┘
 Parser Layout  OCR  Image Font  Style Back  Builder
                 │                      │
                 ↓                      ↓
         ┌──────────────┐      ┌──────────────┐
         │  OCR Backend │      │  Assemble    │
         │  (Protocol)  │      │ (数据组装)    │
         └──┬───┬───┬───┘      └──────────────┘
            │   │   │
            ▼   ▼   ▼
       Trans Olla  vLLM   ← infra/ocr_backend.py
       form  ma
       ers
               ↓
┌─────────────────────────────────────────────────────────┐
│                    Infrastructure                        │
│  ocr_backend │ model_manager │ config │ progress │ ckpt │
└─────────────────────────────────────────────────────────┘
```
...
| 模块 | 文件 | 职责 |
|------|------|------|
| CLI | `cli.py` | 命令行参数解析、输入校验、模式/设备选择 |
| Pipeline Orchestrator | `pipeline.py` | 阶段调度、并行协调、断点续传、单页隔离 |
| Parser | `stages/parser.py` | PDF 解析、页面渲染、嵌入资源提取 |
| Layout | `stages/layout.py` | PP-DocLayout-V3 布局检测 + NMS 后处理 |
| OCR | `stages/ocr.py` | 文字识别（通过 OCR Backend 抽象层调用 GLM-OCR） |
| Image Extractor | `stages/image.py` | 图片区域裁切、嵌入资源 IoU 匹配 |
| Font Matcher | `stages/font.py` | 字体映射表匹配 |
| Style Estimator | `stages/style.py` | 字号/颜色估计 |
| Background | `stages/background.py` | 背景提取（可编辑模式）— 纯色/渐变/LaMA |
| Builder | `stages/builder.py` | Assemble（数据组装）+ PPTX 生成（python-pptx） |
| OCR Backend | `infra/ocr_backend.py` | OCR 推理后端抽象：Transformers / Ollama / vLLM / API |
| Model Manager | `infra/model_manager.py` | 布局检测模型加载（ONNX）、OCR Backend 创建 |
| Config | `infra/config.py` | 全局配置管理 |
| Progress | `infra/progress.py` | Rich 进度条显示 |
...
LayoutResult[]           ← 每页的区域列表 (bbox + semantic label)
    ↓ OCR (via OCR Backend)
OCRResult[]              ← 每个文字区域的识别文本
    ├→ Image Extractor
    │   ExtractedImage[] ← 图片区域裁切 / 嵌入资源匹配
    ├→ Font Matcher
    │   FontMatch[]      ← 字体映射结果
    ├→ Style Estimator
    │   TextStyle[]      ← 字号 + 颜色估计
    └→ Background (仅可编辑模式)
        np.ndarray       ← 去除文字后的干净背景
    ↓ Assemble
SlideContent[]           ← 每页的完整内容描述
    ↓ Builder
output.pptx
```
...
阶段内并行（OCR 完成后的 Image / Font / Style / Background 可并行）：

```
LayoutResult
    ↓
    OCR (GLM-OCR via Backend)     ← GPU/CPU 密集，必须先完成
    ↓
    ├── Image (裁切 + 资源提取)   ← IO 密集
    ├── Font (映射表查询)         ← CPU 轻量
    ├── Style (字号/颜色估计)     ← CPU 轻量
    └── Background (背景重建)     ← CPU 密集
    ↓
...
│       │   ├── __init__.py
│       │   ├── page.py             # PageImage, PageMetadata, BoundingBox, EmbeddedResource
│       │   ├── layout.py           # LayoutRegion, LayoutResult, RegionLabel
│       │   ├── content.py          # OCRResult, ExtractedImage, FontMatch, TextStyle
│       │   └── slide.py            # SlideContent, TextBlock, ImageBlock
│       ├── stages/                 # 管线阶段
│       │   ├── __init__.py
│       │   ├── parser.py           # PDF 解析 + 嵌入资源提取
│       │   ├── layout.py           # 布局检测（PP-DocLayout-V3 ONNX + NMS）
│       │   ├── ocr.py              # 文字 OCR（通过 OCR Backend 抽象层）
│       │   ├── image.py            # 图片提取（嵌入优先 + 裁切回退）
│       │   ├── font.py             # 字体映射表匹配
│       │   ├── style.py            # 字号/颜色估计
│       │   ├── background.py       # 背景提取（纯色/渐变/LaMA）
│       │   └── builder.py          # Assemble + PPTX 生成
│       └── infra/                  # 基础设施
│           ├── __init__.py
│           ├── ocr_backend.py      # OCR 后端抽象（Transformers/Ollama/vLLM/API）
│           ├── model_manager.py    # 模型加载与管理
│           ├── config.py           # 配置管理
│           ├── progress.py         # Rich 进度条
│           └── checkpoint.py       # 页级断点续传
...
| Pillow | 图像处理 | HPND | 必须 |
| numpy | 数组运算 | BSD-3 | 必须 |
| opencv-python-headless | 图像预处理（resize 等） | Apache 2.0 | 必须 |
| PyYAML | 字体映射表解析 | MIT | 必须 |
| click | CLI 框架 | BSD-3 | 必须 |
| rich | CLI 进度条和格式化输出 | MIT | 必须 |
| httpx | API 调用（Ollama / vLLM / Zhipu） | Apache 2.0 | 必须 |

### 7.2 模型推理依赖

| 依赖 | 用途 | 许可 | 必须/可选 |
|------|------|------|----------|
| ONNX Runtime | 布局检测推理（PP-DocLayout-V3） | MIT | 必须 |
| transformers | GLM-OCR Transformers 后端 | Apache 2.0 | 可选（`--device transformers`） |
| torch | Transformers 运行时 | BSD-3 | 可选（随 transformers） |

> **说明**: Ollama 和 vLLM 后端通过 HTTP API 调用，不需要额外 Python 依赖。Transformers 后端需要安装 torch，体积较大（~2GB），因此作为可选依赖。

### 7.3 模型文件

| 模型 | 用途 | 许可 | 管理方式 |
|------|------|------|---------|
| PP-DocLayout-V3 | 布局检测 | Apache 2.0 | ONNX 文件，`~/.noteeditor/models/` |
| GLM-OCR | 文字 OCR | MIT | Transformers: HuggingFace 缓存 / Ollama: `ollama pull` / vLLM: HF 模型 ID |
| LaMA | 复杂背景修复 | Apache 2.0 | 可选，ONNX 文件 |
